    ...
    <div className="space-y-4">
      ...
      */}
      ...
      {excluded.length > 0 ? (
        <Card>
          <CardHeader>
            <CardTitle>Excluded claims</CardTitle>
          </CardHeader>
          <CardContent>
            {/* Every exclusion carries a reason — nothing is silently dropped. */}
            <ul className="space-y-1 text-xs">
              {excluded.map((e) => (
                <li key={e.claimReference} className="flex flex-wrap gap-2">
                  <span className="font-mono">{e.claimReference}</span>
                ...
                <CardContent className="flex flex-wrap items-center justify-between gap-3 pt-6">
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-muted-foreground">
                      {draft.claimCount} claim(s) · PHP {draft.totalAmount.toFixed(2)} ·{' '}
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {draft.status === 'sent' ? (
                      <span className="text-xs text-muted-foreground">
                        Official Order created. Hold expiry routes to review, never
