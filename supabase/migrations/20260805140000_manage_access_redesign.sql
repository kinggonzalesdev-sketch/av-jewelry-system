-- Manage Access redesign (Owner request): a module-based, parent→child permission
-- catalogue. The only new key is `layaway_create` — "Create Layaway" becomes an
-- assignable permission (like Edit/Delete Layaway and the Inventory actions),
-- instead of being hard-coded to the owner/selected_admin roles.
--
-- To preserve existing behaviour (Owner rule: do not break current workflows), every
-- active Admin who can create layaway today is BACKFILLED this grant, so nothing
-- changes for them; the Super Admin can revoke/grant it per member from now on. The
-- Owner holds it implicitly via app_private.has_permission → is_owner().

-- 1. Register the key (FK from staff_permission_grants.permission_key requires it).
insert into public.permissions (key, label, description, is_request_only) values
  ('layaway_create', 'Create Layaway', 'Create a new layaway account (New Entry).', false)
on conflict (key) do nothing;

-- 2. Backfill: grant layaway_create to every ACTIVE selected_admin (they can create
--    layaway today via role). Idempotent; owners need no grant (implicit).
insert into public.staff_permission_grants (staff_profile_id, permission_key)
select sp.id, 'layaway_create'
from public.staff_profiles sp
where sp.is_active
  and sp.role_key = 'selected_admin'
  and not exists (
    select 1 from public.staff_permission_grants g
    where g.staff_profile_id = sp.id and g.permission_key = 'layaway_create'
  );

-- 3. Gate the create RPC on the permission (was: role in owner/selected_admin), so
--    a granted member of ANY role can create and an ungranted one cannot. Verbatim
--    copy of the current function body with ONLY the authorization guard changed.
create or replace function public.create_layaway_account(p_customer_name text, p_items jsonb, p_interest_type text, p_term smallint, p_date_purchased date, p_remarks text, p_payment numeric, p_payment_date date, p_mode_of_payment text, p_admin_id uuid default null::uuid)
 returns jsonb
 language plpgsql
 security definer
 set search_path to ''
...
begin
  if not app_private.has_permission('layaway_create') then
    raise exception 'Not authorized: you do not have permission to create layaway accounts. No record was changed.'
  ...
  insert into public.layaway_ledger (
    ...
    inventory_item_id, customer_id, admin_staff_profile_id, created_by, source_kind, next_due_date
  ) values (
    ...
    p_term, 0, v_total_grams, v_basis, v_rate,
    v_first_iid, null, v_admin, v_staff, 'manual', v_next_due
  ...
  for v_elem in select * from jsonb_array_elements(v_computed) loop
    ...
    values (v_ledger, nullif(v_elem->>'iid', '')::uuid, v_elem->>'code', v_elem->>'name',
      nullif(v_elem->>'grams', '')::numeric, v_elem->>'ptype',
      (v_elem->>'price')::numeric, (v_elem->>'amount')::numeric);
  end loop;
