...
-- and returns any still-reserved item(s) to Active Inventory via an owner-approved RTS
-- review. HARD SAFETY: re-checks Owner AND refuses anything whose status is not
-- 'cancelled', so a live/active order can never be deleted here. Mirrors
-- delete_test_order_and_return_items (20260806180000) but gated on the cancelled status.
create or replace function public.delete_cancelled_order(p_order_id uuid)
returns jsonb
language plpgsql
security definer
set search_path to ''
...
begin
  if not app_private.is_owner() then
    raise exception 'Not authorized: deleting a cancelled order is reserved to the Super Admin (Owner). No record was changed.';
  end if;
  ...
  if v_status <> 'cancelled' then
    raise exception 'Only a Cancelled order can be deleted here — this order is "%". Nothing was changed.', v_status;
  end if;
    ...
    foreach v_item in array v_item_ids loop
      ...
      if v_remaining = 0
        ...
        insert into public.returned_to_stock_reviews (
          ...
          'Cancelled order deleted by Super Admin — item returned to Active Inventory.',
...
revoke execute on function public.delete_cancelled_order(uuid) from public, anon;
