...
data class CaptureRoi(
    ...
    /** Structurally valid: inside [0,1], positive, at least the minimum fraction, and fully on
     *  screen. An invalid box NEVER produces a crop (PHASE 15 — no silent wrong-region OCR). */
    fun isValid(): Boolean =
        left in 0f..1f && top in 0f..1f &&
            width >= MIN_FRACTION && height >= MIN_FRACTION &&
    ...
    fun normalized(): CaptureRoi {
        ...
        val h = height.coerceIn(MIN_FRACTION, 1f)
    ...
    companion object {
        /** A box narrower/shorter than this fraction of the screen is rejected as unusable. */
        const val MIN_FRACTION = 0.05f

        /** A crop smaller than this many pixels on a side is rejected (too little to OCR). */
        const val MIN_PX = 40
